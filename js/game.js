...
class Game {
  ...
  initUI() {
    ...
    const wepDefs = [
      ...
      { name:'SNIPER',  type:'SEMI', dmg:'KO', rateMs:1200, ammo:'48',  trait:'ONE-HIT KILL',  col:'#00e5ff', idx:6 },
    ];
    const buildWepCard = (w, container, selectable) => {
        const rateBar = Math.max(0, Math.min(100, Math.round((1 - w.rateMs / 1200) * 100)));
        const dmgBar  = w.dmg === 'KO' ? 100 : Math.round((w.dmg / 135) * 100);
