...
class Game {
  ...
  buildMegaCity(W) {
    ...
    function makeBuilding(x, z, w, d, h, ci, ni) {
      const bodyMat = new THREE.MeshStandardMaterial({ color: buildingColors[ci % buildingColors.length], roughness: 0.35, metalness: 0.5 });
      ...
      const grp = self.buildAccessibleTower(W, x, z, w, d, h, bodyMat, { win: nm, lamp: neonColors[ni % neonColors.length], glow: neonColors[ni % neonColors.length] });
      ...
      trim.position.set(x, h - 0.2, z); W.add(trim);
      return grp;
    }
